# Vehicle Management System

A professional-grade Console Application built with **C#** and **.NET**, showcasing **N-Layer Architecture** and the **Repository Pattern**.

---

## 🚀 Key Features
* **Vehicle Inventory:** Efficiently manage and track vehicle data.
* **Decoupled Logic:** Separation of business logic, data access, and presentation layers.
* **Repository Pattern:** Implementation of abstracted data persistence for scalability.
* **Clean & Readable Code:** Follows professional naming conventions and SOLID principles.

---

## 🛠️ Technology Stack
* **Language:** C#
* **Framework:** .NET
* **Pattern:** Repository Pattern
* **Architecture:** Layered Architecture (Domain, Repository, UI)

---

## 📁 Project Structure
1. **Vehicle_Domain:** Contains core entities and interface definitions.
2. **Vehicle_Repository:** Handles data logic and XML/Database storage operations.
3. **Vehicle_ConsoleApp:** The UI layer that interacts with the user.
4. **Vehicle_Source:** Manages data sources and common utilities.

---

## 👤 Author
**Syed Monirul Islam**
* **GitHub:** [github.com/Syed-Monirul-Islam](https://github.com/Syed-Monirul-Islam)
* **Email:** syedmonirulislamcu@gmail.com
